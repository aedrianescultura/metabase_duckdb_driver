...
It describes **the current state of this source tree**, not history. When
compatibility changes, edit the list in the same PR as the code change; git
history then records what each release claimed. Publishing a release builds one
image per entry, so this file is what determines coverage.

The Dockerfile's `ARG METABASE_VERSION` names the **primary** version: it is what
`docker build .` with no arguments reproduces, and it is the one entry that also
gets tagged `:latest`. It must be one of the versions in the file — a release
fails before building anything otherwise.
...
`:latest` is never inferred from version numbers, so rebuilding an older combination cannot silently repoint it. The release path sets it for the primary version only; a manual dispatch has to opt in.

Only build combinations known to work together. Metabase 0.63 requires driver **1.5.5.0 or newer**: v63 passes namespaced keywords in connection details, which earlier drivers forward to DuckDB as JDBC properties and fail to connect ([#103](https://github.com/motherduckdb/metabase_duckdb_driver/issues/103)). That fix shipped in [1.5.5.0](https://github.com/motherduckdb/metabase_duckdb_driver/releases/tag/1.5.5.0), so 0.63 must not be paired with 1.5.4.0 or earlier.
...
3. **Add .jar file to a release** attaches `duckdb.metabase-driver.jar`, then its `images` job builds one image per version in the file, tagging `:latest` on the primary.

Step 3's ordering is structural: `images` is gated on `needs: build`, and each image ADDs the driver JAR from its release URL, so no build can start before that JAR is attached. Both JARs are verified to exist before any layers are built, so a bad pin fails fast with a named error instead of a 404 deep in the build.

Pairing an existing driver with an additional Metabase version needs no release — dispatch **Build Container Images** by hand with that Metabase version and the already-published driver version, leaving `tag_latest` unchecked unless it should become the default pull. If the pairing should be permanent, add the version to `metabase_versions.json` so future releases keep building it.
